# AliceLabs Changelog — Memex AliceLabs Edition
...
- **Package metadata:** Renamed PyPI package from `memex-memory` to
  `memex-alicelabs`. Updated `pyproject.toml`, `codemeta.json`,
...
- **P1-1 fix:** Renamed `cogito` -> `memex` in user-facing docstrings,
  print prefixes, and logger names across `src/memex/`. 47 line changes
  ...
  in `init_cmd.py`).
- **P1-2 fix:** Renamed `Memex` -> `Memex` in `bench/*.py` and
