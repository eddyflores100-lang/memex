...
# Memex 0.3.0rc1 — records that keep their correction history

Memex now lets an agent retain a statement, correct it without erasing the old
...
MCP tool names change: use `memex_recall` in place of `memex_query` and update
instructions that call `memex_orient` or `cogito_*` tools. Restart clients so
...
pattern-based. See the [README limitations](../../README.md#is-memex-a-fit).
