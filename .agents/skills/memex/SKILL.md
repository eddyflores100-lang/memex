---
name: memex
description: Use when you need agent memory with a zero-LLM default retrieval path — returning stored passages verbatim via BM25 + dense-vector + reciprocal-rank-fusion — and want an HTTP server plus CLI that can also do compressed-index snapshotting and Claude Code/Codex/Copilot/Gemini/OpenClaw MCP wiring. Local-first, PyPI package `memex-memory`.
license: MIT
compatibility: Requires Python 3.10+; installs via `pip install memex-memory==0.2.0`, executable name is `memex`. Zero-LLM retrieval path needs no external API key; optional filter/extraction tiers need Ollama or an Anthropic/OpenAI-compatible endpoint.
---

# memex

memex is local-first retrieval memory for AI agents and Claude Code. The
...
- Wiring an MCP memory tool into Claude Code, Codex, GitHub Copilot CLI,
  Gemini CLI, or OpenClaw via `memex mcp install`
...
- Proving a model's stored claim is factually true — memex returns what was
  stored, it does not fact-check it

## Quickstart

```bash
pip install "memex-memory==0.2.0"
memex health
```
...
```bash
uvx --from memex-memory memex health
```
...
```bash
uvx --from memex-memory memex query "test query" --limit 2
```
...
- Executable name is `memex`, not `memex-memory` — `uvx memex-memory`
  fails; use `uvx --from memex-memory memex <cmd>`.
- The PyPI project literally named `memex` is unrelated; install
  `memex-memory`.
...
https://github.com/eddyflores100-lang/memex
