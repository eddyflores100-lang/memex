"""
Run Memex's recall_b retrieval against LongMemEval benchmark.
...
Usage:
  python bench/longmemeval_tuned.py --split s --limit 50
  python bench/longmemeval_tuned.py --split m --limit 50
"""
...
def batch_embed(texts: list[str]) -> list[list[float]] | None:
    ...
    # nomic-embed-text has 8192 token context; truncate long texts to ~2000 chars
    truncated = [t[:2000] if len(t) > 2000 else (t if t else "empty") for t in texts]
    max_retries = 8
    for attempt in range(max_retries):
        try:
            body = json.dumps({"model": EMBED_MODEL, "input": truncated}).encode()
            ...
            if attempt < max_retries - 1:
                ...
                print(f"  [embed error, retry {attempt+1}/{max_retries}] {e} (waiting {wait_time}s)", file=sys.stderr)
...
def main():
    ...
    # Warmup: try to embed "warmup" up to 10 times with 3s sleep
    print("Warming up embedding model...")
    warmup_success = False
    for attempt in range(10):
        result = batch_embed(["warmup"])
    ...
    if not warmup_success:
        print("  WARNING: Warmup failed, proceeding anyway", file=sys.stderr)
    ...
    print(f"\n{'='*70}")
    print(f"  TUNED cogito recall_b on LongMemEval_{args.split.upper()}  —  {n} questions")
    print(f"{'='*70}\n")
    ...
    results_data = {
        "test": "tuned_baseline",
        "changes": "cosine_weight=0.55, max_subqueries=12, truncation=2000",
        "split": args.split,
        "n_questions": n,
        "metrics": metrics,
    ...
    results_file = Path(__file__).resolve().parent / "results-tuned-2026-04-15.json"
