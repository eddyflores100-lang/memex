"""
v34 Combined Pipeline: v33 + Runtime escalation rule (no hardcoded hardset).
...
# v34: Runtime confidence thresholds for flagship escalation (replaces hardset)
# Calibrated on score distributions from 65 questions (no gold access).
# Fires on ~10.6% of questions (50/470), close to prior hardset=37 (7.9%).
FLAGSHIP_SCORE_THRESHOLD = 0.65   # escalate if top-1 session score below this
FLAGSHIP_GAP_THRESHOLD = 0.03    # escalate if score gap (top1 - top2) below this
...
def should_escalate_to_flagship(
    ...
    Calibrated thresholds: fires ~50/470 questions (10.6%).
    """
...
def main():
    ...
    print(f"COMBINED PIPELINE v34: {mode}")
    ...
    print(f"\n{'='*70}")
    print(f"  COMBINED PIPELINE v34 (runtime escalation) on LongMemEval_{args.split.upper()}  —  {n_q} questions")
    print(f"{'='*70}\n")
