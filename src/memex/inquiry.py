"""Deterministic, bounded inquiry over Memex's canonical hybrid retriever.
...
INQUIRY_SCHEMA_VERSION = "memex.inquiry-result/v1"
ASK_SCHEMA_VERSION = "memex.structured-ask/v1"
POLICY_SCHEMA_VERSION = "memex.inquiry-policy/v1"
...
def _subject_tokens(query: str) -> tuple[str, ...]:
    named = [
        ...
        if (
            ...
            or token[:1].isupper()
            or token.casefold() in {"sqlite", "postgresql", "memex", "redis"}
        )
...
def _result_record(
    ...
    result = {
        ...
        "evidence_receipt": {
            "schema_version": "memex.inquiry-evidence-receipt/v1",
            "record_id": record_id,
            "source_pointer": pointer,
            "text_sha256": text_sha256,
        },
...
def run_inquiry(
    ...
    *,
    ...
    if retriever is None:
        from memex.recall_hybrid import recall_hybrid
...
def _server_url(server_url: str | None = None) -> str:
    ...
    explicit = os.environ.get("MEMEX_SERVER_URL")
    if explicit:
        return explicit.rstrip("/")
    port = int(
        os.environ.get("MEMEX_PORT")
        or os.environ.get("MEMEX_PORT", "19420")
    )
...
def inquire(
    *,
    ...
    """Submit one inquiry to the local Memex service.
    ...
    try:
        ...
        raise RuntimeError(f"Memex inquiry failed with HTTP {exc.code}: {detail}") from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"Memex server unavailable at {_server_url(server_url)}") from exc
