"""
memex server — HTTP server keeping memory warm in process.
...
Start:
  memex-server                        # uses .cogito.json or env vars
  memex-server --config /path/to.json
  memex-server --port 19420
"""
...
from memex.browse import get_record, recent_records, store_stats
from memex.codex_history import is_codex_history_query, search_codex_history
...
from memex.recall_b import recall_b as do_recall_b
from memex.recall_hybrid import recall_hybrid as do_recall_hybrid
from memex.retrieval_trace import (
    DEFAULT_TRACE_MAX_BYTES,
    RetrievalTrace,
)
from memex.retrieval_telemetry import build_event as build_retrieval_event
from memex.retrieval_telemetry import record as record_retrieval_event
...
from memex.snapshot import _read_snapshot, _snapshot_path
from memex.supersession import (
    filter_ephemera,
    mark_ephemera,
    mark_superseded,
)
from memex.temporal import format_instant
from memex.temporal_recall import carry_payload, overfetch, parse_as_of, temporal_view
...
_RECALL_POOL = concurrent.futures.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="memex-recall"
)
...
class _BoundedThreadingHTTPServer(ThreadingHTTPServer):
    ...
    def process_request(self, request, client_address):
        if not self._inflight.acquire(timeout=5):
            logger.warning("[memex] saturated (%d in-flight); refusing connection",
                           self._MAX_INFLIGHT)
            try:
...
class MemoryHolder:
    ...
    def __init__(self, cfg: dict):
        ...
        self._retry_cooldown_s: float = self._parse_retry_cooldown(
            os.environ.get("MEMEX_MEMORY_RETRY_COOLDOWN_SECS")
        )

    @staticmethod
    def _parse_retry_cooldown(raw: str | None, default: float = 5.0) -> float:
        """Parse MEMEX_MEMORY_RETRY_COOLDOWN_SECS defensively.
...
def _boot(cfg: dict) -> object:
    ...
    # otherwise keeps Python alive after Memex has completed clean shutdown.
...
def _chroma_index_lag(cfg: dict) -> int | None:
    ...
    /health's "queued" only counts memex's own degrade queue (Ollama-down
...
def _run_cogito_hermeneutics(memory: object, cfg: dict, data: dict) -> dict:
    ...
    try:
        ...
        logger.exception("[memex] Cogito Hermeneutics hybrid candidate retrieval failed")
        try:
            ...
            if not history_intent:
                ...
                return _finalize(result)
            logger.exception(
                "[memex] memory retrievers unavailable; continuing bounded Codex history"
            )
            candidates = []
    ...
    if refined_query:
        try:
            ...
            logger.warning(
                "[memex] bounded refinement failed: %s",
                type(exc).__name__,
            )
    ...
    if history_intent:
        ...
        try:
            ...
            history_error = exc
            logger.exception("[memex] bounded Codex history retrieval failed")
            history_results = []
        source_health.append(
            {
...
def make_handler(memory: object, cfg: dict) -> type:
    ...
    # MEMEX_DECOMPOSE_TIMEOUT_SECS: max seconds for /recall sub-query pipeline.
    # Default 8s preserves existing behavior in normal cases; kicks in only on slow-call edges.
    _decompose_timeout: float = float(os.environ.get("MEMEX_DECOMPOSE_TIMEOUT_SECS", 8))
    ...
    _HEALTH_CACHE_TTL_SECS = float(
        os.environ.get("MEMEX_HEALTH_CACHE_TTL_SECS", "10")
    )
    _HEALTH_REFRESH_MAX_AGE_SECS = float(
        os.environ.get("MEMEX_HEALTH_REFRESH_MAX_AGE_SECS", "30")
    )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _json(self, data, status=200):
            ...
            try:
                ...
                self.send_header("Server", "memex-alicelabs")
                self.end_headers()
        ...
        def do_GET(self):
            ...
            try:
                if self.path == "/live":
                    ...
                    if text is None:
                        self._json({"error": "no snapshot — run `memex snapshot` first"}, 404)
                    else:
                ...
                logger.exception("[memex] %s %s failed: %s", self.command, self.path, e)
                try:
            ...
            try:
                ...
                if self.path == "/query":
                    ...
                    try:
                        ...
                        logger.warning(
                            "[memex] /recall decompose timeout (>%ss) for query '%s'; returning vector-only fallback",
                        ...
                        degraded = True
                    print(f"[memex] /recall '{text[:50]}' → {len(memories)} results ({method})", flush=True)
                    memories = temporal_view(
                        filter_ephemera(memories, cfg), memory=memory,
                        index=temporal_index(), as_of=as_of,
                        historical=historical, limit=limit,
                    ...
                    print(
                        f"[memex] /orient '{text[:50]}' mode={result['plan']['mode']} "
                    ...
                    print(f"[memex] /recall_hybrid '{text[:50]}' tier={tier} → {len(memories)} results ({method})", flush=True)
                    self._json({"memories": mark_superseded(memories, cfg), "method": method})
...
def main():
    ...
    # the stack calls basicConfig. MEMEX_LOG_LEVEL overrides per-deployment.
    logging.basicConfig(
    ...
    parser = argparse.ArgumentParser(description="memex memory server")
    ...
    print(f"[memex] Starting server v{__version__} (config: {src})", flush=True)
        ...
        while not stop_event.is_set():
            try:
                ...
                if pending > 0:
                    ...
                    print(
                        f"[memex] queue sweep: replayed={result.get('replayed', 0)} "
    ...
    print(f"[memex] Listening on {args.host}:{port}", flush=True)
    try:
        httpd.serve_forever()
    finally:
        ...
        print("[memex] Stopped cleanly.", flush=True)
