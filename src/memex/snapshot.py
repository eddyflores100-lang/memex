"""
memex snapshot — compressed index layer (alicelabs-memory-style).
...
Usage:
    memex snapshot              # build and write snapshot
    memex snapshot --dry-run    # preview without writing
    memex snapshot --rebuild    # force rebuild even if snapshot exists
"""
...
def snapshot(
    ...
    """
    from memex.recall import _resolve_filter_endpoint  # avoid circular at module level

    if not rebuild:
        existing = _read_snapshot(cfg)
        if existing:
            print(f"[memex snapshot] Snapshot already exists at {_snapshot_path(cfg)}")
            print("[memex snapshot] Use --rebuild to regenerate.")
            return existing
    ...
    print(f"[memex snapshot] Sampling memories (n={n})...")
    memories = _sample_memories(memory, user_id, n)
    if not memories:
        raise RuntimeError("No memories in store. Run `memex seed` first.")
    print(f"[memex snapshot] Sampled {len(memories)} memories. Calling {model}...")

    text = _build_snapshot(memories, endpoint, token, model, timeout)
    lines = text.count("\n") + 1
    tokens_est = len(text.split())
    print(f"[memex snapshot] Generated {lines} lines (~{tokens_est} tokens).")

    if dry_run:
        print("[memex snapshot] DRY RUN — not writing.\n")
        print(text)
        return text

    path = _write_snapshot(text, cfg)
    print(f"[memex snapshot] Written to {path}")
    return text
