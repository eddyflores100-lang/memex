...
from memex.temporal import SCHEMA, apply_temporal, content_sha256, parse_instant
...
_RECONCILED_FLAG = "_memex_reconciled"
    ...
    try:
        ...
        if full:
            ...
            for record_id, meta in zip(ids, metadatas):
                ...
                repaired += 1
        logger.warning(
            "[memex] temporal index healed from store (%s): %s records",
            "rebuild" if full else "additive", repaired,
        )
        return repaired
    except Exception as exc:  # noqa: fail-open — a stale cache is reported, never fatal
        logger.warning("[memex] temporal index could not be reconciled: %s", exc)
        return None
...
def _resolve(hit: Mapping[str, Any], memory: Any, index: Any) -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug("[memex] temporal lookup skipped: %s", exc)
    if record_id:
    ...
    try:
        ...
        if index is not None:
            try:
                ...
                logger.warning("[memex] temporal index unreadable: %s", exc)
        ...
        logger.warning("[memex] temporal view unavailable: %s", exc)
