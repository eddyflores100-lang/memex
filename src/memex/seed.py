"""
memex seed — bulk-seed the memory store from markdown/text files.
...
Usage:
    memex seed ~/memory/                          # seed all .md files
    memex seed ~/memory/ ~/notes/sessions/        # multiple dirs
    memex seed ~/memory/ --dry-run                # show facts without writing
    memex seed ~/memory/ --force                  # re-seed even unchanged files
    memex seed ~/memory/ --glob "*.md"            # filter by pattern
    memex seed ~/memory/ --add                    # use /add (mem0 extraction) instead
...
def _check_server(base_url: str) -> int:
    try:
        ...
        raise RuntimeError(f"memex server not reachable at {base_url}") from e
...
def seed(
    ...
    if not use_add:
        ...
        if endpoint:
            print(f"[memex seed] Curation model: {model} @ {endpoint}")
        else:
            print("[memex seed] No curation endpoint — storing chunks verbatim via /store (no extraction)")
    ...
    if not dry_run:
        try:
            ...
            print(f"[memex seed] Server OK — {count_before} memories before seeding")
    ...
    print(f"[memex seed] {len(all_files)} file(s) — write path: {mode}\n")
    ...
    if not dry_run:
        ...
        try:
            ...
            print(f"\n[memex seed] Done. {count_after} memories in store.")
