...
SCHEMA_VERSION = "memex.retrieval-telemetry/v1"
FAILURE_CATEGORIES = {
    "memory_not_invoked",
    "query_too_broad",
    "wrong_time_scope",
...
def _enabled() -> bool:
    raw = os.environ.get("MEMEX_RETRIEVAL_TELEMETRY", "1").strip().lower()
    if raw in {"1", "true", "yes", "on"}:
        return True
    if raw in {"0", "false", "no", "off"}:
        return False
    raise ValueError("MEMEX_RETRIEVAL_TELEMETRY must be a boolean")


def _path() -> Path:
    configured = os.environ.get("MEMEX_RETRIEVAL_TELEMETRY_LOG")
    if configured:
