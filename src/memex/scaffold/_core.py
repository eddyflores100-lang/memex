"""Memex Scaffold v0.1.0 — drift-safe QA wrapper for retrieved memory.
...
Versioning: scaffold tokens are bracketed with [MEMEX-SCAFFOLD-v0.1.0] markers so
...
SCAFFOLD_OPEN = f"[MEMEX-SCAFFOLD-{SCAFFOLD_VERSION}]"
SCAFFOLD_CLOSE = f"[/MEMEX-SCAFFOLD-{SCAFFOLD_VERSION}]"
...
def wrap_system_prompt(qtype: str, top_score: float | None = None) -> str:
    """Return a complete Memex-scaffold system prompt for a given qtype.
...
def is_scaffolded(text: str) -> bool:
    """Detect whether text already contains a Memex scaffold (any version)."""
    return "[MEMEX-SCAFFOLD-" in text


def strip_scaffold(text: str) -> str:
    """Remove all Memex scaffold sections from text. Idempotent for un-scaffolded text."""
    import re
    pat = re.compile(
        r"\[MEMEX-SCAFFOLD-[^\]]+\].*?\[/MEMEX-SCAFFOLD-[^\]]+\]",
        re.DOTALL,
    )
