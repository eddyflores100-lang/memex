"""Memex Scaffold preflight validator — must pass before any scaffold ships.
...
class PreflightReport:
    ...
    def summary(self) -> str:
        ...
        out = [f"Memex Scaffold Preflight: {status}"]
        if self.metrics:
...
def preflight(scaffold_text: str, max_tokens: int = 200) -> PreflightReport:
    ...
    open_count = scaffold_text.count("[MEMEX-SCAFFOLD-")
    close_count = scaffold_text.count("[/MEMEX-SCAFFOLD-")
