"""memex init — install + start the memex service so memory is "on" automatically.

Cross-platform:
- macOS: launchd plist at ~/Library/LaunchAgents/ai.eddyflores100-lang.memex-server.plist
- Linux: systemd user unit at ~/.config/systemd/user/memex-server.service
...
PORT = 19420
SERVICE_LABEL = "ai.eddyflores100-lang.memex-server"
...
Description=Memex agent memory server
...
def _server_bin() -> str:
    """Locate the memex-server entry point installed by pip."""
    bin_path = shutil.which("memex-server")
    if bin_path:
        return bin_path
    # Fallback: look in the same dir as the active python
    candidate = Path(sys.executable).parent / "memex-server"
    if candidate.exists():
        return str(candidate)
    raise RuntimeError(
        "memex-server entry point not found on PATH. "
        "Reinstall AliceLabs Memex from its tagged GitHub source; "
...
def _detect_existing_service(label: str, port: int) -> dict | None:
    ...
    if result.returncode == 0:
        ...
        #   launchctl list ai.eddyflores100-lang.memex-server -> PID 74977
...
def _install_macos(
    ...
    if not force and not dry_run:
        ...
        if collision:
            print(
                f"ERROR: memex-server already running from {collision['binary']} "
            ...
            print(
                "  To upgrade this installation, run: memex init --force",
                file=sys.stderr,
            )
            if label == SERVICE_LABEL and port == PORT:
                print(
                    "  To run multiple memex instances, use: "
                    "memex init --label <custom-label> --port <port>",
                    file=sys.stderr,
                )
            return 1
    ...
    if dry_run:
        ...
        log_path = Path.home() / ".memex" / "server.log"
...
def _install_linux(
    ...
    if port is None:
        port = PORT
    if label is None:
        label = "memex-server"  # systemd uses .service suffix, not a separate label concept
    ...
    for cmd in (
        ...
        if result.returncode != 0:
            print(
                ...
                f"        or fall back to running `memex-server` manually under your process manager",
                file=sys.stderr,
            )
            return 1
...
def _install_fallback(uninstall: bool = False, **kwargs) -> int:
    ...
    if uninstall:
        # Best-effort: kill any running memex-server
        subprocess.run(["pkill", "-f", "memex-server"], check=False)
        print("attempted to stop any running memex-server (no auto-start was configured)")
        return 0
...
def cmd_init(args) -> int:
    """Install + start memex-server as a system service.
    ...
    print(f"installing memex-server as a {system} service...")
    if system == "Darwin":
        rc = _install_macos(
            force=force,
            dry_run=dry_run,
    ...
    if _health_check(timeout_s=10.0, port=check_port):
        print(f"✓ memex-server is up at http://127.0.0.1:{check_port}")
        print(f"  log: {Path.home() / '.memex' / 'server.log'}")
        print()
        print("next steps:")
        print("  memex health                  # confirm")
        print("  memex watch ~/notes           # auto-ingest a directory")
        print("  memex mcp install             # wire up Claude Code")
        return 0
    else:
        print("✗ service installed but /health did not respond within 10s")
        print(f"  check log: {Path.home() / '.memex' / 'server.log'}")
        return 2
