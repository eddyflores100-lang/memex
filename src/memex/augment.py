"""memex.augment — the one-line caller helper.

For users who want "give me memory + scaffold + answer" in one call instead of
wiring three separate functions. Pure wrapper; uses the running memex-server
for retrieval and the user's own LLM client for generation.

Example:

    from memex.augment import augment
    ...
    response = augment(
        ...
        llm_call=lambda system, user: client.messages.create(
            model="claude-sonnet-4-5",  # use any current Claude Messages API model
            system=system,
...
def _recall(query: str, limit: int = 5) -> tuple[str, float | None]:
    """Retrieve memories from memex-server. Returns (formatted_context, top_score)."""
    ...
    try:
        ...
        raise RuntimeError(
            f"memex-server unreachable at {_server_url()}. "
            f"Run `memex init` to install + start the service. ({e})"
        ) from e
    ...
    *,
    ...
    Args:
        ...
        llm_call: a callable taking (system_prompt, user_message) and returning
            the LLM's response text. The user supplies this so memex stays
    ...
    Raises:
        RuntimeError: if memex-server is unreachable.
    """
