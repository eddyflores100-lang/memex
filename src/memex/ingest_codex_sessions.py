"""Local Codex session ingestion for Memex.
...
``memex sessions ingest --source codex`` (or ``--source all``). Prompt-history
...
from memex.ingest_claude_sessions import (
    COGITO_SESSIONS_DIR,
    _excluded,
    _get_collection,
    _included,
...
def _stable_chroma_id(session_id: str) -> str:
    return str(uuid.uuid5(uuid.NAMESPACE_URL, f"memex:codex:{session_id}"))
