"""
memex calibrate — one-time vocabulary bridge extraction.
...
Usage:
    memex calibrate
    memex calibrate --sample 300 --dry-run
"""
...
def calibrate(
    ...
    print(f"[memex calibrate] Sampling memories (n={n})...")
    memories = _sample_memories(memory, user_id, n)
    if not memories:
        raise RuntimeError("No memories in store. Run `memex seed` first.")
    print(f"[memex calibrate] Sampled {len(memories)} memories. Calling {model}...")

    vocab_map = _build_vocab_map(memories, endpoint, token, model, timeout)
    print(f"[memex calibrate] {len(vocab_map)} vocab mappings extracted.")

    if dry_run:
        print("[memex calibrate] DRY RUN — not writing config.")
    ...
    print(f"[memex calibrate] Written to {path}")
    return vocab_map
