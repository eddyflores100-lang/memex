"""
memex sessions — searchable, purge-able session memory (MVP-A).
...
PRIVACY_NOTICE = (
    ...
    "(readable only by your user). Run `memex sessions purge` to remove. " + BACKUP_BOUNDARY
)
...
def cmd_purge(args) -> int:
    ...
    from memex.ingest_claude_sessions import _get_collection  # lazy: avoids chromadb import at CLI start
...
def _is_first_run() -> bool:
    ...
    from memex.ingest_claude_sessions import _load_ledger
    try:
...
def cmd_ingest(args) -> int:
    from memex.ingest_claude_sessions import ingest as ingest_claude
    from memex.ingest_codex_sessions import ingest as ingest_codex
    ...
    if default_window and _is_first_run():
        full_history_command = "memex sessions ingest --all"
        if source != "claude":
            full_history_command = f"memex sessions ingest --source {source} --all"
        print(
...
def cmd_search(args) -> int:
    ...
    """
    from memex.recall_recent import recall_recent_work
...
def cmd_list(args) -> int:
    if args.since and not _valid_iso_date(args.since):
        print("Error: --since must be a valid YYYY-MM-DD date")
        return 2
    from memex.ingest_claude_sessions import _get_collection
    col = _get_collection()
    entries = col.get(where={"mem_type": "session"}, include=["metadatas"])
    metas = entries.get("metadatas", []) or []
...
def cmd_stats(args) -> int:
    ...
    if not metas:
        if args.since:
            ...
            print("No sessions indexed. Run `memex sessions ingest`.")
        return 0
