"""
memex.ingest_claude_sessions — Claude Code session ingestion for memex.

Reads ~/.claude/projects/*/SESSION_ID.jsonl, extracts role-structured user+assistant
turn pairs, and stores each session as ONE session-typed memory in memex's ChromaDB.

Usage:
    python3 -m memex.ingest_claude_sessions [--since YYYY-MM-DD] [--dry-run]
...
USER_ID = "agent"
MAX_TEXT_CHARS = 2000          # matches memex's safe truncation limit
...
def _load_exclude() -> list[str]:
    """Deny-list from MEMEX_SESSIONS_EXCLUDE (comma-separated) env var."""
    raw = os.environ.get("MEMEX_SESSIONS_EXCLUDE", "")
...
def _load_include() -> list[str]:
    """Allow-list from MEMEX_SESSIONS_INCLUDE (comma-separated) env var."""
    raw = os.environ.get("MEMEX_SESSIONS_INCLUDE", "")
    return [t.strip() for t in raw.split(",") if t.strip()]
...
def _load_mode() -> str:
    ...
    MEMEX_SESSIONS_MODE env var, else the 'sessions_mode' key in
    ...
    env = os.environ.get("MEMEX_SESSIONS_MODE", "").strip().lower()
    if env in valid:
        return env
...
def _make_ingest_hash(
    ...
    # Preserve the original Claude hash byte-for-byte so an upgrade does not
    # invalidate the existing ledger and duplicate every previously indexed chat.
    prefix = "" if (
        session_source == "claude_code" and capture_completeness == "NATIVE_MESSAGES"
    ) else f"{session_source}:{capture_completeness}:"
    payload = prefix + session_id + json.dumps([
...
def _store_session(
    ...
    write = col.upsert if upsert else col.add
    write(
        ids=[chroma_id],
        documents=[text],
...
def ingest(
    ...
    """
    Ingest Claude Code sessions into memex.

    mode: privacy posture, 'opt-out' (default) or 'opt-in'. Falls back to
    _load_mode() (MEMEX_SESSIONS_MODE env / config.json) when not passed.
    exclude: project-name substrings to skip (deny-list), used in opt-out mode.
    Falls back to the MEMEX_SESSIONS_EXCLUDE env var when not passed.
    include: project-name substrings to allow (allow-list), used in opt-in mode.
    Falls back to the MEMEX_SESSIONS_INCLUDE env var when not passed.
    ...
    if mode == "opt-in" and not include:
        print(
            "opt-in mode: no projects included yet — set MEMEX_SESSIONS_INCLUDE=... "
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Ingest Claude Code sessions into memex memory.",
...
Examples:
    ...
    python3 -m memex.ingest_claude_sessions --since 2026-04-11 --dry-run

    # Actually ingest all sessions from last 7 days
    python3 -m memex.ingest_claude_sessions --since 2026-04-11

    # Ingest everything (may take a while)
    python3 -m memex.ingest_claude_sessions
""",
    )
    parser.add_argument(
        "--since",
    ...
    print(f"[memex-ingest] {mode} — scanning {CLAUDE_PROJECTS}")
    if since:
        print(f"[memex-ingest] Since: {args.since}")
