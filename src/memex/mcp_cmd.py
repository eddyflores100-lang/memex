"""Wire memex into a supported agent client's MCP configuration.
...
def _is_memex_codex_entry(entry: dict) -> bool:
...
def _cmd_codex_install(args) -> int:
    ...
    if not codex_bin:
        print(
            ...
            "  install Codex, then rerun: memex mcp install --client codex",
            file=sys.stderr,
        )
        return 1
    ...
    if existing is not None:
        if _is_memex_codex_entry(existing):
            print("Codex MCP server 'memex' is already configured; nothing to change")
            return 0
        if not args.force:
            print(
                "error: a non-memex Codex MCP server named 'memex' already exists\n"
    ...
    print(result.stdout.strip() or "registered Codex MCP server 'memex'")
    print("next: restart Codex, then use /mcp to confirm the memex tools")
    return 0
...
def _cmd_codex_uninstall() -> int:
    ...
    if existing is None:
        if rc != 0 and "No MCP server named" not in error:
            print(error or "error: could not inspect Codex MCP configuration", file=sys.stderr)
            return rc
        print("no 'memex' MCP server registered in Codex; nothing to uninstall")
        return 0
    if not _is_memex_codex_entry(existing):
        print(
            "error: Codex MCP server 'memex' does not appear to belong to Memex; refusing to remove it",
            file=sys.stderr,
        )
        return 1
    result = subprocess.run(
    ...
    if result.returncode != 0:
        ...
        return result.returncode
    print(result.stdout.strip() or "removed Codex MCP server 'memex'")
    return 0
...
# ``copilot mcp add``. Memex edits it atomically with a backup so the host CLI
...
def copilot_server_entry() -> dict:
    """The exact Copilot CLI stdio entry Memex registers."""
    return {
        "type": "stdio",
...
def _is_memex_server_path(value: object) -> bool:
    """Return whether a configured argument points at a Memex MCP server.

    An exact match against this install is the common case. But a user who
    registered Memex from another environment — a venv since rebuilt, pipx,
    ...
    both the packaged file name and its `memex` package directory keeps
...
def _cmd_copilot_install(args) -> int:
    if not MCP_SERVER_FILE.exists():
        print(
            ...
            "  this install appears incomplete; reinstall AliceLabs Memex "
    ...
    if existing == entry:
        ...
        return 0
    if existing is not None and not _is_memex_copilot_entry(existing) and not args.force:
        print(
            f"error: a non-memex Copilot CLI MCP server named '{MCP_SERVER_NAME}' already exists in {config_path}\n"
    ...
    print()
    print("next: restart Copilot CLI, then run /mcp list to see the memex server")
    print("  /mcp show memex shows its status and the tools it exposes")
    print("  Copilot may ask you to approve the memex tools on first use")
    return 0
...
def _cmd_copilot_uninstall(args) -> int:
    ...
    if not isinstance(mcp_servers, dict) or MCP_SERVER_NAME not in mcp_servers:
        ...
        return 0
    if not _is_memex_copilot_entry(mcp_servers[MCP_SERVER_NAME]) and not getattr(args, "force", False):
        print(
            f"error: Copilot CLI MCP server '{MCP_SERVER_NAME}' in {config_path} does not appear "
            "to belong to Memex; refusing to remove it\n"
...
# `//` and `/* */` comments. A rewrite of that file by Memex would silently
...
# that mirrors Gemini's own. Memex never writes that file itself.
# ---------------------------------------------------------------------------
...
def gemini_server_entry() -> dict:
    """The exact stdio entry `gemini mcp add` writes for Memex.
...
def _is_memex_gemini_entry(entry: object) -> bool:
    """Return whether a Gemini MCP entry launches a Memex MCP server."""
    if not isinstance(entry, dict):
        return False
    args = entry.get("args")
    if not isinstance(args, list) or len(args) != 1:
        return False
    return _is_memex_server_path(args[0])
...
def _gemini_entry_matches(entry: object) -> bool:
    ...
    interpreter path must match this install, not merely some Memex one."""
    if not isinstance(entry, dict):
        return False
...
def _gemini_preflight(args) -> tuple[str, str, Path] | None:
    ...
    if not gemini_bin:
        print(
            ...
            "memex mcp install --client gemini",
            file=sys.stderr,
        )
        return None
...
def _cmd_gemini_install(args) -> int:
    ...
    if existing is not None:
        if _gemini_entry_matches(existing):
            ...
            return 0
        if not _is_memex_gemini_entry(existing) and not args.force:
            print(
                f"error: a non-memex Gemini CLI MCP server named '{MCP_SERVER_NAME}' already "
...
def _cmd_gemini_uninstall(args) -> int:
    ...
    if existing is None:
        ...
        return 0
    if not _is_memex_gemini_entry(existing) and not getattr(args, "force", False):
        print(
            f"error: Gemini CLI MCP server '{MCP_SERVER_NAME}' in {config_path} "
            f"({scope} scope) does not appear to belong to Memex; refusing to remove it\n"
            f"  entry: {_safe_entry_summary(existing)}\n"
            "  use --force to remove it anyway.",
            file=sys.stderr,
        )
...
# Memex neither writes it nor parses it. OpenClaw's own read-only CLI is the
# authority on what is in it:
#
#     openclaw mcp show memex --json   # that server's definition, or exit 1
...
# specific config file. Memex pins it for every delegated call -- the reads
...
def openclaw_add_arguments() -> list[str]:
    """The exact documented ``openclaw mcp add`` arguments Memex delegates."""
    return [
        "mcp",
        "add",
        MCP_SERVER_NAME,
...
def _looks_like_python_interpreter(command: str) -> bool:
    ...
    that is the one Memex itself has ever actually written via
...
def _mentions_memex_server(node: object) -> bool:
    ...
    exactly one argument naming our script -- is what Memex itself ever
    writes. Matching a looser one would let a foreign server be recognized
    as Memex's own, and then get installed over or uninstalled without
...
def _openclaw_state(openclaw_bin: str, config_path: Path) -> tuple[str, object, str]:
    """Ask OpenClaw what ``mcp.servers.memex`` currently is.

    Memex never parses the config itself. JSON5 is a superset of JSON, so a
    ...
    that cannot read the file is exactly how a user's foreign ``memex`` entry
    ...
    if (
        ...
        state = _OC_OURS if _mentions_memex_server(payload) else _OC_FOREIGN
    ...
    return (_OC_OURS if _mentions_memex_server(entry) else _OC_FOREIGN), entry, ""


def _openclaw_entry_matches(entry: object) -> bool:
    """Whether a read-back OpenClaw entry is exactly the launch we asked for.

    Stricter than ``_mentions_memex_server``: ownership only proves the
    entry is Memex's, not that this particular install landed. A prior
...
def _openclaw_missing_cli(action: str) -> int:
    print(
        ...
        f"  install OpenClaw, then rerun: memex mcp {action} --client openclaw",
        file=sys.stderr,
    )
    return 1
    ...
    print(
        "\n".join(
            [
                ...
                f"  an existing entry cannot be ruled out, so Memex will not {action} blindly.",
...
def _cmd_openclaw_install(args) -> int:
    ...
    if state == _OC_FOREIGN and not args.force:
        print(
            f"error: a non-memex OpenClaw MCP server named '{MCP_SERVER_NAME}' already exists "
            f"in {config_path}\n"
            f"  entry: {_safe_entry_summary(existing)}\n"
            "  refusing to overwrite. Use --force to replace it.",
            file=sys.stderr,
    ...
    if not _openclaw_entry_matches(entry):
        ...
        print(
            ...
            f"{config_path} does not match what Memex requested\n"
...
def cmd_mcp_install(args) -> int:
    ...
    # Refuse to overwrite a non-memex entry under the memex name. Look at
    # both command and args — our own previous install puts the memex path in
    ...
    if existing and not args.force:
        ...
        if "memex" not in existing_blob and "mcp_server.py" not in existing_blob:
            print(
    ...
    print(f"  the memex tools will appear under the prefix mcp__{MCP_SERVER_NAME}__*")
    return 0
