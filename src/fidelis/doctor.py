"""fidelis doctor — diagnostic command for prerequisites and runtime health.
...
def _check_store_writable(store_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        test_file = p / ".fidelis_write_test"
...
def _check_service_installed() -> dict[str, Any]:
    if sys.platform == "darwin":
        ...
        return {
            ...
            "fix": "Run `fidelis init` to install the launchd service" if not ok else None,
        }
    elif sys.platform.startswith("linux"):
        unit = Path.home() / ".config/systemd/user/fidelis-server.service"
        ok = unit.exists()
        return {
            "check": "service_installed",
            "ok": ok,
            "platform": "Linux",
            "unit_path": str(unit),
            "fix": "Run `fidelis init` to install the systemd service" if not ok else None,
        }
    else:
        return {
            "check": "service_installed",
...
def _check_server_alive(port: int) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "port": port,
            "error": str(e),
            "fix": f"Start the server: `fidelis-server` or `fidelis init` if not installed",
        }
...
def _check_mcp_clients() -> dict[str, Any]:
    ...
    return {
        ...
        "clients": clients,
        "fix": "Run `fidelis mcp install --client <name>` to configure a client" if not any(c["configured"] for c in clients.values()) else None,
    }
...
def run_doctor() -> int:
    ...
    if "--json" in sys.argv:
        ...
        print(f"Fidelis doctor v{__version__}")
        print("=" * 50)
        for r in results:
