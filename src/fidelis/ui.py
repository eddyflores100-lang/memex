"""fidelis ui — local web UI for browsing memories.
...
Usage:
    fidelis ui                    # start UI on port 19421
    fidelis ui --port 8080        # use custom port
    fidelis ui --no-browser       # don't open browser automatically

The UI talks to the running fidelis-server (default :19420) via HTTP.
...
<title>Fidelis Memory — AliceLabs UI</title>
<style>
...
<body>
<div class="header">
  <h1>Fidelis Memory <span class="badge">AliceLabs</span></h1>
...
async function exportBackup() {
  ...
  a.download = 'fidelis-backup-' + Date.now() + '.json';
  a.click();
}
...
class UIHandler(BaseHTTPRequestHandler):
    """HTTP handler that serves the UI and proxies API calls to fidelis-server."""
    ...
    def _proxy_get(self, path: str) -> None:
        """Proxy a GET request to the fidelis-server."""
        try:
            ...
            self._send_json({"error": f"fidelis-server unreachable: {e}"}, 503)
        except Exception as e:
            self._send_json({"error": str(e)}, 500)

    def _proxy_post(self, path: str) -> None:
        """Proxy a POST request to the fidelis-server."""
        try:
...
def cmd_ui(args) -> int:
    ...
    # Check if fidelis-server is running
    try:
        req = urllib.request.Request(f"{_server_url()}/health", method="GET")
        with urllib.request.urlopen(req, timeout=3) as resp:
            health = json.loads(resp.read())
        print(f"[fidelis-ui] Connected to fidelis-server (v{health.get('version')}, {health.get('count')} memories)")
    except Exception as e:
        print(f"[fidelis-ui] Warning: fidelis-server not reachable at {_server_url()}", file=sys.stderr)
        print(f"[fidelis-ui] Start it first: `fidelis init` or `fidelis-server`", file=sys.stderr)
        return 1

    server = ThreadingHTTPServer((host, port), UIHandler)
    url = f"http://{host}:{port}"
    print(f"[fidelis-ui] Listening on {url}")
    print(f"[fidelis-ui] Press Ctrl+C to stop")
...
def register_parser(sub) -> None:
    """Register the `fidelis ui` subcommand."""
    p_ui = sub.add_parser(
        "ui",
