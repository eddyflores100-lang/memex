"""fidelis backup — export/import memories for backup and migration.

AliceLabs proprietary addition. Provides:
  - `fidelis backup export <path>`  — export all memories to a JSON file
  - `fidelis backup import <path>`  — import memories from a JSON file
  - `fidelis backup list`           — list available backups in default dir
...
def cmd_export(args) -> int:
    ...
    print(f"Connecting to fidelis-server at {_base_url()}...", file=sys.stderr)
    health = _get("/health")
    ...
    if args.output:
        ...
        out_path = backup_dir / f"fidelis-backup-{ts}.json"
...
def cmd_import(args) -> int:
    ...
    if data.get("version") != "fidelis-backup/v1":
...
def cmd_list(args) -> int:
    ...
    backups = sorted(backup_dir.glob("fidelis-backup-*.json"), reverse=True)
    if not backups:
...
def register_parsers(sub) -> None:
    ...
    p_export.add_argument("output", nargs="?", help="Output path (default: ~/.cogito/backups/fidelis-backup-<timestamp>.json)")
