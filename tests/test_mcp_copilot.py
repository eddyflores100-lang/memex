"""Regression tests for `memex mcp install|uninstall --client copilot`.
...
def test_install_refuses_foreign_entry_without_force(tmp_path, capsys):
    config = tmp_path / "mcp-config.json"
    foreign = {"type": "stdio", "command": "npx", "args": ["some-other-memex"], "tools": ["*"]}
    config.write_text(json.dumps({"mcpServers": {"memex": foreign}}))

    assert cmd_mcp_install(_args(str(config))) == 1
    assert _read(config)["mcpServers"]["memex"] == foreign
    assert not _backups(config)
    assert "refusing to overwrite" in capsys.readouterr().err

    assert cmd_mcp_install(_args(str(config), force=True)) == 0
    assert _read(config)["mcpServers"]["memex"] == copilot_server_entry()


def test_install_updates_stale_memex_entry_in_place(tmp_path):
    """An older memex entry (different interpreter / missing tools) is ours; refresh it."""
    config = tmp_path / "mcp-config.json"
    stale = {"type": "local", "command": "/old/python", "args": [str(MCP_SERVER_FILE)]}
    config.write_text(json.dumps({"mcpServers": {"memex": stale}}))
...
def test_uninstall_removes_only_memex(tmp_path):
    config = tmp_path / "mcp-config.json"
    assert cmd_mcp_install(_args(str(config))) == 0
    data = _read(config)
    ...
    after = _read(config)
    assert "memex" not in after["mcpServers"]
...
def test_install_under_copilot_home_creates_directory(tmp_path, monkeypatch):
    ...
    assert "memex" not in _read(config)["mcpServers"]
...
def test_cli_accepts_copilot_client(tmp_path, monkeypatch):
    config = tmp_path / "mcp-config.json"
    monkeypatch.setattr(sys, "argv", ["memex", "mcp", "install", "--client", "copilot", "--settings", str(config)])
    with pytest.raises(SystemExit) as exc:
        cli.main()
    assert exc.value.code == 0
    assert _read(config)["mcpServers"]["memex"] == copilot_server_entry()

    monkeypatch.setattr(sys, "argv", ["memex", "mcp", "uninstall", "--client", "copilot", "--settings", str(config)])
    with pytest.raises(SystemExit) as exc:
        cli.main()
    assert exc.value.code == 0
    assert "memex" not in _read(config)["mcpServers"]
...
def test_install_creates_a_new_config_owner_only(tmp_path):
    """A config Memex creates may later gain an `env` block with API tokens."""
    config = tmp_path / "mcp-config.json"
    assert cmd_mcp_install(_args(str(config))) == 0
...
def test_two_mutations_in_one_second_keep_both_restore_points(tmp_path, monkeypatch):
    ...
    assert any("memex" in snapshot["mcpServers"] for snapshot in contents)
...
@pytest.mark.parametrize(
    "path",
    [
        "/opt/homebrew/lib/python3.12/site-packages/memex/mcp_server.py",
        "~/.local/pipx/venvs/memex-memory/lib/python3.11/site-packages/memex/mcp_server.py",
    ],
)
def test_a_memex_install_from_another_environment_is_recognized_as_ours(path):
    assert _is_memex_server_path(path)
...
def test_foreign_servers_are_not_mistaken_for_ours(path):
    assert not _is_memex_server_path(path)
...
def test_install_refreshes_an_entry_written_by_another_environment(tmp_path):
    ...
    stale = {
        ...
        "args": ["/opt/homebrew/lib/python3.12/site-packages/memex/mcp_server.py"],
        "tools": ["*"],
    }
    config.write_text(json.dumps({"mcpServers": {"memex": stale}}))
...
def test_uninstall_force_removes_a_foreign_entry(tmp_path, capsys):
    ...
    config = tmp_path / "mcp-config.json"
    foreign = {"type": "stdio", "command": "npx", "args": ["other"]}
    config.write_text(json.dumps({"mcpServers": {"memex": foreign, "keep": {"type": "http"}}}))
